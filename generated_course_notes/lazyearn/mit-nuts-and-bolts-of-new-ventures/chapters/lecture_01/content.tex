...
\section{The Entrepreneur's First Questions}
...
\begin{enumerate}
  \item How do we start?
  \item What, exactly, do we do first?
  \item What problem are we solving?
  \item Does the proposed solution really answer that problem?
  \item Who cares about the idea, specifically, and who is the customer?
  \item How will the venture make money?
  \item How long will it take to get to market, what will it cost, and what resources will be required?
  \item What legal entity, if any, should be formed?
  \item How can the idea be protected, if protection is the right objective?
  \item Will cofounders be needed, and how will the relationship be structured?
  \item How will negotiation enter, with employees, consultants, customers, advisors, and investors?
  \item How do we discover what we do not know?
\end{enumerate}
...
\paragraph{Answer.} \(R\) is reality and \(E\) is expectations. The lecture briefly records a revealing false start: some Sloan students suggest that \(R\) must be revenue and \(E\) must be expenses. Hadzima accepts that reading as a subset and then enlarges it. The equation is not an accounting identity. It is a more general rule for disappointment and satisfaction.
...
\begin{remark}
The lecture jokes that if we could drive expectations to zero, we might produce infinite happiness. We should not literalize the joke into a usable model. The mathematical content we actually need is comparative: if expectations rise while reality does not, the quotient worsens.
\end{remark}
...
This is why the lecture pauses over \textit{mens et manus}. Ideas and execution belong together. Yet even that pair is not sufficient.
...
\begin{remark}
The graph is schematic. The arrows are straight, unscaled, and not tied to a marked crossing time. The lecture explicitly says that whether the lines cross is not the point. The point is that the relative balance changes, and with it the conversation about legitimacy, structure, and equity.
\end{remark}
...
\paragraph{SpeechCo.} This is the lecture's success case of alignment. The idea was strong. The timing was good because compute power had finally arrived. Execution improved by restricting the system to a domain of language use rather than trying to solve everything at once. And the people had spent enough time together before launch to form a functioning team.

\paragraph{VideoCo.} This case initially looks like an execution failure in the making. The founders seem incapable even of choosing a company name. The payoff is that they were not merely naming the company; they were choosing a strategy. The DIVA/Avid connection reveals that what looked like hesitation had a directional logic.

\paragraph{HIV Co.} Here the idea, timing, and scientific credibility were all strong. The weakness was people. Cofounder conflict drained the venture even in the presence of first-rate backers and advisers. This is Hadzima's cleanest case of a venture weakened primarily by internal human structure.

\paragraph{NanoCo.} This is the lecture's timing case. NanoFuel had technical promise and environmental appeal, but the market arrived in the wrong price regime. The lecture even gives a back-of-the-envelope threshold:
\begin{align}
...
\begin{tikzpicture}[x=0.92cm,y=0.92cm,>=Latex,font=\small]
  ...
  \draw[->,thick] (2.25,3.95) -- (0.04,3.95);
  \node[anchor=west,align=left] at (2.32,4.03) {Mission/Vision\\Statement};
  \node[anchor=west,text=red!80!black] at (4.80,4.03) {1 sentence/paragraph};

  \draw[->,thick] (2.25,3.22) -- (0.10,3.22);
  \node[anchor=west,align=left] at (2.32,3.22) {Elevator Pitch};

  \draw[->,thick] (2.25,2.38) -- (0.26,2.38);
  \node[anchor=west,align=left] at (2.32,2.38) {Executive Summary};

  \draw[->,thick] (2.25,1.08) -- (0.70,1.08);
  \node[anchor=west,align=left] at (2.32,1.08) {Pitch Deck};
\end{tikzpicture}
...
From there the lecture moves by successive compressions. First: create value and capture value. Then: why this, why now, why this team, and why won't this work? Then: ideas, execution, timing, and people. Finally: mission statement, elevator pitch, executive summary, and pitch deck. Each compression is followed by examples, because the lecture never allows a framework to float free of lived ventures.
